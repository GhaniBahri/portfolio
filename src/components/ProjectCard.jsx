...
function ProjectCard({image, name, link}) {
  return (
    <div className='w-80 h-80 relative flex flex-col justify-between items-center group border overflow-hidden'>
        ...
        <div className='flex justify-center items-center absolute left-0 top-0 w-full h-full bg-[rgba(0,0,0,40%)] -translate-x-full group-hover:translate-x-0 transition-all duration-1000 ease-in-out'>
            <Link href={`${link}`} className='text-white font-medium text-base m-auto'>Visit link!</Link>
        </div>
        <span className='text-black font-medium text-lg text-center w-full'>
            {name}
        </span>
    </div>
  )
